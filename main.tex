...
\quad The library implements the novel idea of \textit{computational frameworks}, which are higher level entities that embody both data structures and algorithms together. The use of computational frameworks as parallel software design primitives alleviates the software parallelization task for a wide class of applications. We prototyped the library on the Olden benchmark suite. The parallel library version consistently outperforms the sequential version hitting 5-6x speedups on the major benchmarks.\newline\null
}
