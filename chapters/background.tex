...
\quad Nowadays the parallelism is pervasive. Parallel hardware is omnipresent across the whole wide spectrum of computing systems. In order to exploit all the hardware capabilities the software has to be parallel as well. And thus, every computer scientist and software developer would benefit from having an insight into the area. Nonetheless, the topics are extremely difficult, require a serious time investment and a great deal of knowledge in various subdomains. It is not always practical to expect from an average programmer to possess such a deep expertise. For that reason we propose a solution aimed at alleviating the challenging task of manual software parallelization. Our solution consists of two components. We describe them in chapters \ref{assistant} and \ref{computational_frameworks}.\newline\null
\quad In this chapter we accumulate the material, which lays the ground for descriptions of our proposed solutions in chapters \ref{assistant} and \ref{computational_frameworks}. We relied on parallel computing tutorials from Lawrence Livermore National Laboratory (LLNL) \cite{llnl_computing} to prepare the material. Section \ref{parallel_computing_importance} stresses the importance of parallel computing and software parallelization. Section [] presents the challenges of manual and then automatic software parallelization. These challenges form the ground for our assistant solution to grow on. Then, in section [] we discuss the problem of data structure choice and how it affects software parallelization. We present a thorough literature review on the topic of Data Centric Parallelization (DCP) dealing with the task of automatic data structure recognition. Our review concludes that automatic techniques are still an unsolved grand challenge. Data structures are often inseparable with algorithms the former support. Our computational frameworks we propose in chapter \ref{computational_frameworks} grow on the inseparability fact and are a blend of data structures and algorithms. Furthermore, the modern software design and engineering tasks are extreamly rich and complex topics. In sections [] and [] we talk about imperative, functional and object-oriented programming paradigms, as well as various OOP software design patterns. Our computational frameworks take the best principles and alleviate the task of software architecture design for an average programmer.        

\section{The importance of parallel computing}
\label{background_parallel_computing_importance}
...
\quad The problem of software parallelization is extremely complex and multi-faceted. There are various approaches to the problem, but all of them have their pros and cons. Although, the process of software parallelization has characteristically been a very manual task, which is time-consuming and error-prone, there are also automatic and machine learning based approaches to the problem. In this section we highlight inherent challenges of these approaches. The solution we propose aims to address these challenges. 
